...
const TopSellers = () => {
    ...
    return (
        <div className='bg-white p-5 mx-5 mt-[5rem] border w-[23rem] rounded h-fit'>
                ...
                {authors.map((author, index) => (
                    <li key={index} className='flex items-center justify-between mb-4'>
                        ...
                        <button onClick={() => handleFollowClick(index)}
                            className={`ml-4 py-1 px-3 rounded ${author.isFollowing ? 'bg-red-500 text-white' : 'bg-black text-white'}`}
